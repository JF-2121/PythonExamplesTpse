...
- I can export per-face cap meshes to STL files from `extract_and_mesh_caps_02.py`.
- I can add `pv.OFF_SCREEN = True` to scripts for robust headless runs.
- I can create a `pyproject.toml` or a pinned `requirements-dev.txt` with exact tested versions.
